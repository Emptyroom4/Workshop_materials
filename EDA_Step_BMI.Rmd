...
This dataset includes information on daily step counts, Body Mass Inedex(BMI), and gender for 1,786 individuals.
...
We notice that the data types of ID and gender are incorrect. 
```{r}
...
We computed Pearson’s correlation between steps and BMI. The correlation is negative, which looks natural, but the effect size is not large.  
...
Both correlations are negative and fairly similar. The association between BMI and steps is slightly stronger in the male group than in the female group.


Next, we examine how BMI varies with steps using a linear regression model, which quantifies how BMI changes on average as step count changes.
...
We see that each additional step is associated with an average decrease of about 0.000392 BMI units.
...
Was this surprising?
Our model gave a statistically significant result — yet we completely missed the gorilla hiding in the data! 
This shows why visualization should come first.

The Steps and BMI dataset comes from the 2020 article A Hypothesis Is a Liability, in which 
the authors created an artificial dataset to demonstrate how prior beliefs shape our attention.

In the experiment, one group was given three specific hypotheses to test (e.g., 
“There is a difference in the mean number of steps between women and men.”), 
while another group was asked to explore the data freely. 
The hypothesis-free group was 2.6 times more likely to notice the hidden gorilla in the dataset than the group that was given hypotheses.

Another classic example highlighting the importance of visualization is Anscombe’s quartet.
